...
int _VSMVirPageStatusBufferCount;
StructuredBuffer<VirPageStatus> _VSMVirPageStatusBuffer;

TEXTURE2D(_VSMPhysicalTexture);
SAMPLER(sampler_VSMPhysicalTexture);

float VSMMainLightRealtimeShadow(float3 positionWS)
{
    float dist = distance(positionWS, _VSMMainCameraWorldPos.xyz);
    int startMipLevel = floor(log2(max(dist, 0.001)) * _VSMDistanceSensitivity);
    startMipLevel = clamp(startMipLevel, 0, _VSMMipCount - 1);
    
    float4 positionLS = mul(_VSMLightMatrix, float4(positionWS, 1.0));
    float3 ndc = positionLS.xyz / positionLS.w; // xy in [-1, 1] z in [1, 0] (reversed-z)
    float2 virUV = ndc.xy * 0.5 + 0.5;
    if (virUV.x < 0.0 || virUV.x > 1.0) return 0.0;
    if (virUV.y < 0.0 || virUV.y > 1.0) return 0.0;
    
    // 1. Initial Mip Params
    int mipLevel = startMipLevel;
    int2 maxGridSize = int2(_VSMPageGridParams.xy);
    int2 virPageGridSize = max(int2(1,1), maxGridSize >> mipLevel);
    int pagesPerLayer = maxGridSize.x * maxGridSize.y; 
    int mipLayerOffset = mipLevel * pagesPerLayer; 
    float2 virPageUV = virUV * virPageGridSize;
    int2 virLoc = (int2)virPageUV;
    int virIdx = mipLayerOffset + (virLoc.y * maxGridSize.x + virLoc.x);
    
    // 2. Page Table Lookup
    if (virIdx >= _VSMVirPageStatusBufferCount || virIdx < 0) return 1.0;
    VirPageStatus virPageStatus = _VSMVirPageStatusBuffer[virIdx];
    if (virPageStatus.Status == VIR_PAGE_STATUS_UNLOADED) return 1.0;
    if (virPageStatus.Status == VIR_PAGE_STATUS_INDIRECT)
    {
        mipLevel = virPageStatus.AncestorMip;
        virPageGridSize = max(int2(1,1), maxGridSize >> mipLevel);
        virPageUV = virUV * virPageGridSize;
    }
    
    // Status is Loaded OR Indirect
    float2 pageUVOffset = frac(virPageUV);
    float2 phyLoc = float2(virPageStatus.PhyPageLoc.x, virPageStatus.PhyPageLoc.y);
    float2 phyPageGridSize = _VSMPageGridParams.zw;
    float2 phyUV = (phyLoc + pageUVOffset) / phyPageGridSize;
    phyUV.y = 1.0 - phyUV.y;
    const float closestDepth = SAMPLE_TEXTURE2D_LOD(_VSMPhysicalTexture, sampler_VSMPhysicalTexture, phyUV, 0).r;
    float currDepth = ndc.z;
    float shadow = currDepth < (closestDepth - 0.001) ? 1.0 : 0.0;
    return 1.0 - shadow;
}
// [VSM] END
...
half MainLightShadow(float4 shadowCoord, float3 positionWS, half4 shadowMask, half4 occlusionProbeChannels)
{
    ...
    // [VSM] BEGIN
    half realtimeShadow = VSMMainLightRealtimeShadow(positionWS);
