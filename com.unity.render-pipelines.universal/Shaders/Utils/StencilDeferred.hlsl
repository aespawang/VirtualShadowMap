...
#define UNIVERSAL_STENCIL_DEFERRED
    ...
    half4 DeferredShading(Varyings input) : SV_Target
    {
        ...
        // VSM DebugVis: Virtual Page
        {
            float dist = distance(posWS.xyz, _VSMMainCameraWorldPos.xyz);
            int mipLevel = floor(log2(max(dist, 0.001)) * _VSMDistanceSensitivity);
            mipLevel = clamp(mipLevel, 0, _VSMMipCount - 1);
            
            float4 positionLS = mul(_VSMLightMatrix, float4(posWS.xyz, 1.0));
            float3 ndc = positionLS.xyz / positionLS.w; // xy in [-1, 1] z in [1, 0] (reversed-z)
            float2 virUV = ndc.xy * 0.5 + 0.5;
            if (virUV.x < 0.0 || virUV.x > 1.0) return half4(color, alpha);
            if (virUV.y < 0.0 || virUV.y > 1.0) return half4(color, alpha);
            int2 virPageGridSize = int2(_VSMPageGridParams.xy) >> mipLevel;
            float2 virPageUV = virUV * virPageGridSize;
            int2 virLoc = (int2)virPageUV;
            color *= (virLoc.x + virLoc.y) % 2 == 0 ? float4(1, 1, 1, 1) : float4(0.5, 0.5, 0.5, 1);
            color *= HsvToRgb(float3((mipLevel + 1.0) / _VSMMipCount, 1, 1));
        }
